// The suggestion surface: when the worker's turn lands, the rook
...
// their own instead is the signal that rook's bid wasn't good enough.
...
func (s *server) agentSuggest(id string) (*suggestRec, *sayErr) {
	...
	if s.llm == nil {
		return nil, &sayErr{409, "no rook-agent key — suggestions are off"}
	}
	...
	if rec := s.suggests[h]; rec != nil {
		...
		<-rec.done
		if rec.Err != "" {
			return nil, &sayErr{502, "the rook agent did not answer: " + rec.Err}
		}
		return rec, nil
	}
