...
func TestSayRPCTranslatesRefusals(t *testing.T) {
	...
	_, err := r.Say(ctx, connect.NewRequest(&verav1.SayRequest{Id: "stranger", Text: "hi"}))
	if connect.CodeOf(err) != connect.CodeNotFound {
		t.Fatalf("a gone agent must be NotFound: %v", err)
	}
	_, err = r.Say(ctx, connect.NewRequest(&verav1.SayRequest{Id: "sess-live", Text: "   "}))
	if connect.CodeOf(err) != connect.CodeInvalidArgument {
		t.Fatalf("an empty message must be InvalidArgument: %v", err)
	}
	_, err = r.Say(ctx, connect.NewRequest(&verav1.SayRequest{Id: "sess-live", Text: "hi", Perm: "sudo"}))
	if connect.CodeOf(err) != connect.CodeInvalidArgument {
...
func TestInterruptRPCTranslatesRefusals(t *testing.T) {
	...
	_, err := r.Interrupt(ctx, connect.NewRequest(&verav1.InterruptRequest{Id: "stranger"}))
	if connect.CodeOf(err) != connect.CodeNotFound {
		t.Fatalf("a gone agent must be NotFound: %v", err)
	}
	_, err = r.Interrupt(ctx, connect.NewRequest(&verav1.InterruptRequest{Id: "sess-live"}))
...
func TestWatchBoardStreamsFramesOnChange(t *testing.T) {
	...
	path, h := verav1connect.NewVeraServiceHandler(&veraRPC{s: s})
	mux.Handle(path, h)
	srv := httptest.NewServer(mux)
	defer srv.Close()

	client := verav1connect.NewVeraServiceClient(srv.Client(), srv.URL)
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	stream, err := client.WatchBoard(ctx, connect.NewRequest(&verav1.WatchBoardRequest{}))
	if err != nil {
		t.Fatal(err)
	}
	if !stream.Receive() {
...
func TestWatchAgentStreamsSnapshotThenDelta(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	stream, err := client.WatchAgent(ctx, connect.NewRequest(&verav1.WatchAgentRequest{Id: "sess-live", Raw: true}))
	if err != nil {
		t.Fatal(err)
	}
	if !stream.Receive() {
