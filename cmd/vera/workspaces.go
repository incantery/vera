// Scratch workspaces: directories vera itself creates, offers, and
...
// managed parent (~/vera-scratch) — creation and deletion are bounded
...
// names the folder as vera's, and doubles as the standing "protected
...
const scratchMarkerText = `This scratch workspace was created by vera.
...
func defaultScratchParent() string {
	...
	return filepath.Join(home, "vera-scratch")
}
...
func (sc *scratchStore) remove(name string) error {
	...
	if _, err := os.Stat(filepath.Join(path, scratchMarker)); err != nil {
		// No marker, no delete: vera only removes what vera made.
		return errors.New("that is not a vera scratch workspace")
	}
