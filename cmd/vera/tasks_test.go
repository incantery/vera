...
func TestTaskAdoptCarriesTheAgentAndItsTitle(t *testing.T) {
	st := &taskStore{dir: t.TempDir()}
	a, err := st.adopt("root-1", "Create local CLI alternative to Rook host", "vera-cloud", time.Now())
	...
	if a.Log[0].Actor != "vera" {
		t.Fatalf("adoption is vera's act, on the record: %+v", a.Log)
	}
}
...
func TestEscalationLandsAsWaitingWithTheAsk(t *testing.T) {
	...
	for _, e := range got.Log {
		if e.Actor == "vera" && strings.Contains(e.Text, "escalated") {
			found = true
		}
	}
	if !found {
...
func TestScratchWorkspaceLifecycle(t *testing.T) {
	...
	// Only marked directories are removable: vera deletes what vera made.
