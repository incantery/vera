// vera — the smallest useful piece of vera: run one binary, get
// the membrane. It watches this machine's Claude Code sessions the way
// vera's plugins do (the transcripts are the witness; nothing is
...
//
// No vera, no cloud, no account. The judge wants an OpenAI-compatible
...
func main() {
	...
	if key == "" && *apiBase == "" {
		s.notice = "no vera-agent key — set $OPENAI_API_KEY or write " + *keyFile + " (drives, digests and phrasing are off; watching works)"
	} else {
		s.llm = &drive.LLM{
	...
	// the same binary; requireKey guards the /vera. prefix too.
	// POST-only: every Connect RPC is a POST, and a method-less
	// pattern would fight the SPA's "GET /" under Go 1.22 mux rules.
	rpcPath, rpcHandler := verav1connect.NewVeraServiceHandler(&veraRPC{s: s})
	...
	if loopbackOnly(*addr) {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "vera: cannot mint a key ("+err.Error()+") — refusing to serve the LAN unlocked")
			os.Exit(1)
		}
		handler = requireKey(mux, key)
		_, port, _ := strings.Cut(*addr, ":")
		fmt.Println("vera: serving beyond loopback — a key guards the API. Open with:")
		for _, u := range lanURLs(port) {
			fmt.Printf("vera:   %s/?key=%s\n", u, key)
		}
		fmt.Println("vera: (or open any of them bare and type the key at the login screen)")
	}
	if s.notice != "" {
...
// already paid for these), and what vera's OWN calls cost — judge,
...
// phrased by the vera agent (unless verbatim), the phrased message
