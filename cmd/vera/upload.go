// Image attachments: the browser pastes, vera stores, the worker
...
func (s *server) handleUpload(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		httpErr(w, 400, "not an image vera knows ("+kind+") — png, jpeg, gif, webp")
		return
	}
...
// vera itself stored for THIS agent — anything else is refused, not
// guessed at.
func (s *server) checkImages(root string, images []string) string {
	...
	for _, p := range images {
		...
		if filepath.Dir(clean) != dir {
			return "an attachment must be one vera stored for this agent"
		}
