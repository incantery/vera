#!/bin/sh
# vera demo — the five-minute flow from QUICKSTART.md, driven through
...
#
# HONESTY UP FRONT: ~/vera-demo is a SCRATCH WORKSPACE, not an
# isolated sandbox. Work mode scopes which tools the agent may use
# (via claude's --allowedTools; vera passes it as one =-joined token,
...
#
# Needs: vera running (default: loopback, keyless), the claude CLI
# logged in, python3, go. Costs: two claude seed/drive runs, well
# under a dollar total. For a LAN vera: VERA=http://host:4770
# VERA_KEY=<key> ./demo.sh
set -eu

VERA=${VERA:-http://127.0.0.1:4770}
WS="$HOME/vera-demo"

api() { # api METHOD PATH [JSON-BODY]
	if [ -n "${VERA_KEY:-}" ]; then
		curl -sf -X "$1" -H "Authorization: Bearer $VERA_KEY" "$VERA$2" ${3:+-d "$3"}
	else
		curl -sf -X "$1" "$VERA$2" ${3:+-d "$3"}
	fi
}
...
# One tiny claude turn makes the directory fleet-shown: vera only
# offers directories it has seen a Claude session in.
if ! api GET /api/tasks | grep -q '"dir": *"vera-demo"'; then
	(cd "$WS" && claude -p "Reply OK — this seeds the vera demo workspace." >/dev/null)
	sleep 3
fi

echo "== 1. workspace selection: only what the fleet has shown =="
REPO=$(api GET /api/tasks | jget "[r['cwd'] for r in d['repos'] if r['dir']=='vera-demo'][0]")
echo "   offered: $REPO"

echo "== 2. capture (inbox, unassigned, nothing spent) =="
T=$(api POST /api/tasks '{"text":"In this demo workspace, work in three phases, STOPPING after each phase to ask permission before the next. Phase 1: create go.mod (module verademo) and greet.go with a Greet(name string) string function. Phase 2: create greet_test.go with a real test and run go test. Phase 3: request authorization to ALSO delete DO-NOT-DELETE.txt - do not delete anything without explicit authorization."}' | jget "d['task']['id']")
echo "   captured: $T"
...
echo "   The demo agent's transcripts:     ~/.claude/projects/-${HOME#/}-vera-demo (rename slashes to dashes)"
